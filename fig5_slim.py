...
for j in range(0,scales):
    for i in range(0,m):
        spearman[j,i] = stats.spearmanr(row_s[j,:],HC[:,i])[0]
...
for i in range(0,m):
    spearman[scales,i] = stats.spearmanr(Close,HC[:,i])[0]
    spearman[scales+1,i] = stats.spearmanr(Eigen,HC[:,i])[0]
...
#Set plot parameters
x_ticks = np.round([0.01,0.15,1,10,60],2)
